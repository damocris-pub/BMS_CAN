...
udll.crc16.argtypes = [c_char_p, c_uint, c_ushort]
udll.crc16.restype = c_ushort
#uint32_t crc32(uint8_t *buffer, uint32_t len, uint32_t start)
udll.crc32.argtypes = [c_char_p, c_uint, c_uint]
...
udll.can_sendPacketData.argtypes = [c_ushort, c_char_p]
...
kPacketLen = 128        #options: 8, 16, 32, 64, 128, 256, 512
kBmsFile = "bms.dfu"
...
if ((fileLen & (kPacketLen - 1)) != 0):
    ...
    sys.exit(1)
buffer = (c_ubyte * fileLen).from_buffer_copy(content)
...
if (success < 0):
    ...
    sys.exit(1)
print(f"LV BMS FW bootloader's version is {resp[3]}.{resp[2]}.{resp[1]}, build is {resp[0]}, HW is {resp[4]}")

success = udll.can_getApplicationVerCmd(c_ubyte(kBMSAddr), resp)
if (success < 0):
    print("could not fetch LV BMS application's version")
    sys.exit(1)
print(f"LV BMS FW application's version is {resp[4]}.{resp[3]}.{resp[2]}, build is {(resp[1] << 8) | resp[0]}")
...
if (success < 0):
    ...
    sys.exit(1)
print(f"LV BMS's hardware build date is year:20{resp[0]:02}, month:{resp[1]:02}, day:{resp[2]:02}, batch no: {(resp[4] << 8) | resp[3]}")
...
while (seq <= fileLen//kPacketLen):     #TODO: test with different sequence order
    ...
    if (success < 0 or ((resp[1] << 8) | resp[0]) != seq):
        print(f"try to set packet sequence num {seq} failed")
        sys.exit(1)
    success = udll.can_sendPacketData(c_ushort(kPacketLen), buffer + (seq-1) * kPacketLen)
    if (success < 0):
        print(f"try to send packet data for {seq} failed")
        sys.exit(1)
    packetCrc = udll.crc16(buffer + (seq-1) * kPacketLen, c_uint(kPacketLen), c_ushort(0xFFFF))
    print(f"packet seq {seq} 's crc is 0x{packetCrc:04x}")
    success = udll.can_verifyPacketDataCmd(c_ushort(kPacketLen), c_ushort(packetCrc))
    if (success < 0):
...
if (kCrcType == 0): #crc16
    fileCrc = udll.crc16(buffer, c_uint(fileLen), c_ushort(0xFFFF))
else:   #crc32
    fileCrc = udll.crc32(buffer, c_uint(fileLen), c_uint(0))
